...
$pdf->Cell(185,6,'DIAGNOSTICO',0,0,'C');
...
$pdf->Cell(64.25,6,'Num diagnostico',1,0,'C',1);
...
$numDiagnostico = $_POST['Num_diagnostico'];

$sqlDiagnostico = "SELECT * FROM diagnostico WHERE Num_diagnostico = '$numDiagnostico' ORDER BY Num_diagnostico ASC";
$query = mysqli_query($conexion, $sqlDiagnostico);

while ($dataRow = mysqli_fetch_array($query)) {
    // Primera fila con los datos principales
    $pdf->Cell(64.25,10,($dataRow['Num_diagnostico']),1,0,'C');
    ...
    $pdf->Cell(37,20,'Padecimiento',1,0,'C',1); // Cabecera
    $pdf->SetFont('helvetica','',10); // Cambia la fuente a normal
    $pdf->Cell(220,20,($dataRow['Padecimiento']),1,1,'L'); // Datos (181 = 35 + 20 + 23 + 103)



    $pdf->SetFont('helvetica','B',12); // Cambia la fuente a negritas
    // Tercera fila para "Sintomas"
    $pdf->Cell(37,20,'Sintomas',1,0,'C',1); // Cabecera
    $pdf->SetFont('helvetica','',10); // Cambia la fuente a normal
    $pdf->Cell(220,20,($dataRow['Sintomas']),1,1,'L'); // Datos



    $pdf->SetFont('helvetica','B',12); // Cambia la fuente a negritas
    // Cuarta fila para "Comentarios"
    $pdf->Cell(37,20,'Comentarios',1,0,'C',1); // Cabecera
    $pdf->SetFont('helvetica','',10); // Cambia la fuente a normal
    $pdf->Cell(220,20,($dataRow['Comentarios']),1,1,'L'); // Datos
}
